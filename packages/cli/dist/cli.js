...
Examples:
  ...
  levr auth login --device-code   # Device flow for SSH/headless`
	...
	loader: async () => {
		const { loginHandler } = await import("./loginHandler-BYNsaZAk.js");
		return loginHandler;
	}
});
...
Examples:
  levr auth status`
	...
	parameters: {},
	loader: async () => {
		const { statusHandler } = await import("./statusHandler-C_QwnKxB.js");
...
Examples:
  ...
  levr push ./report.json   # uses LEVR_TOKEN env var, default team`
	...
	loader: async () => {
		const { pushHandler } = await import("./pushHandler-93QoRgpa.js");
		return pushHandler;
	}
});
...
Examples:
  ...
  levr import ./cases.csv --team-id <uuid> --mapping-file mapping.json --yes   # CI replay`
	...
	loader: async () => {
		const { importHandler } = await import("./importHandler-D02q-DFV.js");
...
Examples:
  levr workspace list`
	...
	parameters: {},
	loader: async () => {
		const { listHandler } = await import("./listHandler-B1riQOWj.js");
		return listHandler;
	}
});
...
Examples:
  levr workspace select <workspace-id>`
	...
	loader: async () => {
		const { selectHandler } = await import("./selectHandler-aSV5_0B9.js");
