...
Examples:
  ...
  levr auth login --device-code   # Device flow for SSH/headless`
	...
	loader: async () => {
		const { loginHandler } = await import("./loginHandler-Cuf5Y6_g.js");
		return loginHandler;
	}
});
...
Examples:
  levr auth status`
	...
	parameters: {},
	loader: async () => {
		const { statusHandler } = await import("./statusHandler-BWTMdVkY.js");
...
Examples:
  ...
  levr push ./report.json   # uses LEVR_TOKEN env var, default team`
	...
	loader: async () => {
		const { pushHandler } = await import("./pushHandler-CeuPhyKB.js");
		return pushHandler;
	}
});
...
Examples:
  ...
  levr import ./cases.csv --team-id <uuid> --mapping-file mapping.json --yes   # CI replay`
	...
	loader: async () => {
		const { importHandler } = await import("./importHandler-CR-lOqcD.js");
...
Examples:
  levr workspace list`
	...
	parameters: {},
	loader: async () => {
		const { listHandler } = await import("./listHandler-VRZS-s8e.js");
		return listHandler;
	}
});
...
Examples:
  levr workspace select <workspace-id>`
	...
	loader: async () => {
		const { selectHandler } = await import("./selectHandler-BeTwholg.js");
